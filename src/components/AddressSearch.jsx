...
export default function AddressSearch({ onSearch, loading, compact }) {
  ...
  const valid = street.trim() && city.trim() && country.trim()
  ...
  if (compact) return (
    <form onSubmit={submit}>
      <div className="liquid-glass" style={{ borderRadius: 16, padding: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {[{v:street,s:setStreet,p:'Street address',f:2},{v:city,s:setCity,p:'City',f:1},{v:state,s:setState,p:'State/Province',f:1},{v:country,s:setCountry,p:'Country',f:1}].map(({v,s,p,f}) => (
    ...
    <form onSubmit={submit}>
      <div className="liquid-glass-strong" style={{ borderRadius: 20, padding: 24 }}>
        <div style={{ marginBottom: 12 }}>
          <label style={{ display:'block', fontSize:11, color:'rgba(255,255,255,0.4)', letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6, fontFamily:"'Barlow',sans-serif" }}>Street address</label>
          <input value={street} onChange={e=>setStreet(e.target.value)} placeholder="e.g. 123 Maple Street" disabled={loading} style={inputStyle} onFocus={focus} onBlur={blur} />
        </div>
