...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "signup") {
      ...
      return res
        .status(200)
        .json({ token: accessToken, refreshToken, userId: id, email: key, is_pro: !!is_pro, is_admin })
    }
    ...
    if (action === "signin") {
      ...
      return res
        .status(200)
        .json({ token: accessToken, refreshToken, userId: user.id, email: key, is_pro: !!user.is_pro, is_admin })
    }
    ...
    if (action === "reset-password") {
      ...
      return res
        .status(200)
        .json({ token: accessToken, refreshToken, userId: user.id, email: user.email, is_pro: !!user.is_pro, is_admin })
    }
    ...
    if (action === "usage") {
      ...
      const result = await db.execute({
        sql: "SELECT analyses_used, is_pro, cerebras_key FROM users WHERE email = ?",
        args: [payload.email],
      })
      ...
      return res
        .status(200)
        .json({ analyses_used: user.analyses_used, is_pro: !!user.is_pro, has_own_key: !!user.cerebras_key })
    }
    ...
    if (action === "get-key") {
      ...
      const result = await db.execute({
        sql: "SELECT cerebras_key, cerebras_key_iv FROM users WHERE email = ?",
        args: [payload.email],
      })
    ...
    if (action === "delete-account") {
      ...
      if (clientRt) {
        try {
          await getRedis().del(`rt:${clientRt}`)
        } catch {}
      }
      return res.status(200).json({ success: true })
    }
    ...
    if (action === "notify-pro") {
      ...
      await db.execute(`INSERT OR IGNORE INTO pro_waitlist (email, created_at) VALUES (?, ?)`, [
    ...
    if (action === "webhook") {
      ...
      if (STRIPE_WEBHOOK_SECRET) {
        return res.status(501).json({
          error: "Webhook signature verification requires a raw-body endpoint. Configure /api/stripe-webhook instead.",
        })
      }
      const db = getDb()
    ...
    if (action === "create_team" || action === "createTeam") {
      const auth = verify(req.headers.authorization)
      ...
      const db = getDb()
      await db.execute({ sql: "INSERT INTO teams (id, name) VALUES (?, ?)", args: [teamId, name] })
      await db.execute({ sql: "UPDATE users SET team_id = ? WHERE email = ?", args: [teamId, auth.email] })
      return res.status(200).json({ id: teamId, name })
    }

    // ── Teams: list ────────────────────────────────────────────────────────
    if (action === "list_teams" || action === "listTeams") {
      const auth = verify(req.headers.authorization)
      if (!auth?.email) return res.status(401).json({ error: "Unauthorized" })
      const db = getDb()
      const userResult = await db.execute({ sql: "SELECT team_id FROM users WHERE email = ?", args: [auth.email] })
      const teamId = userResult.rows?.[0]?.team_id
      if (!teamId) return res.status(200).json({ teams: [] })
      const teamResult = await db.execute({
        sql: "SELECT id, name, created_at FROM teams WHERE id = ?",
        args: [teamId],
      })
      return res.status(200).json({ teams: teamResult.rows || [] })
    }

    // ── API Keys: create ────────────────────────────────────────────────────────
    if (action === "create_api_key" || action === "createApiKey") {
      const auth = verify(req.headers.authorization)
      if (!auth?.email) return res.status(401).json({ error: "Unauthorized" })
      const db = getDb()
      const userResult = await db.execute({ sql: "SELECT team_id FROM users WHERE email = ?", args: [auth.email] })
      const teamId = userResult.rows?.[0]?.team_id
      if (!teamId) return res.status(400).json({ error: "No team found. Create a team first." })
      const countResult = await db.execute({
        sql: "SELECT COUNT(*) as count FROM api_keys WHERE team_id = ? AND revoked_at IS NULL",
        args: [teamId],
      })
      if ((countResult.rows?.[0]?.count || 0) >= 5) return res.status(400).json({ error: "Max 5 API keys per team" })
      ...
      await db.execute({
        sql: "INSERT INTO api_keys (id, team_id, hashed_key, name, daily_usage_count, last_reset_date) VALUES (?, ?, ?, ?, 0, ?)",
        args: [keyId, teamId, hashedKey, name, today],
      })
      return res.status(200).json({ id: keyId, key: apiKey, name, dailyLimit: 200 })
    }

    // ── API Keys: list ────────────────────────────────────────────────────────
    if (action === "list_api_keys" || action === "listApiKeys") {
      const auth = verify(req.headers.authorization)
      if (!auth?.email) return res.status(401).json({ error: "Unauthorized" })
      const db = getDb()
      const userResult = await db.execute({ sql: "SELECT team_id FROM users WHERE email = ?", args: [auth.email] })
      const teamId = userResult.rows?.[0]?.team_id
      if (!teamId) return res.status(200).json({ apiKeys: [] })
      const keysResult = await db.execute({
        sql: "SELECT id, name, created_at, revoked_at, daily_usage_count, last_reset_date FROM api_keys WHERE team_id = ? ORDER BY created_at DESC",
        args: [teamId],
      })
      const today = new Date().toISOString().split("T")[0]
      const keys = (keysResult.rows || []).map((k) => ({
        ...
        dailyUsage: k.daily_usage_count || 0,
        lastResetDate: k.last_reset_date,
        remaining: k.revoked_at ? 0 : k.last_reset_date !== today ? 200 : Math.max(0, 200 - (k.daily_usage_count || 0)),
      }))
      return res.status(200).json({ apiKeys: keys })
    }

    // ── API Keys: revoke ────────────────────────────────────────────────────────
    if (action === "revoke_api_key" || action === "revokeApiKey") {
      const auth = verify(req.headers.authorization)
      if (!auth?.email) return res.status(401).json({ error: "Unauthorized" })
      ...
      await db.execute({
        sql: "UPDATE api_keys SET revoked_at = datetime('now') WHERE id = ? AND team_id = ?",
        args: [keyId, teamId],
      })
      return res.status(200).json({ success: true })
    }

    // ── API Keys: validate (for API middleware) ────────────────────────────
    if (action === "validate_api_key" || action === "validateApiKey") {
      const apiKey = req.headers["x-api-key"] || req.body?.apiKey
      if (!apiKey) return res.status(401).json({ error: "API key required" })
      const hashedKey = createHash("sha256").update(apiKey).digest("hex")
      const db = getDb()
      const keyResult = await db.execute({
        sql: "SELECT id, team_id, revoked_at, daily_usage_count, last_reset_date FROM api_keys WHERE hashed_key = ?",
        args: [hashedKey],
      })
      ...
      if (key.last_reset_date !== today) {
        await db.execute({
          sql: "UPDATE api_keys SET daily_usage_count = 0, last_reset_date = ? WHERE id = ?",
          args: [today, key.id],
        })
        key.daily_usage_count = 0
      }
      if ((key.daily_usage_count || 0) >= 200) return res.status(429).json({ error: "Daily limit exceeded (200/day)" })
      await db.execute({
        sql: "UPDATE api_keys SET daily_usage_count = daily_usage_count + 1 WHERE id = ?",
        args: [key.id],
      })
      return res.status(200).json({ valid: true, teamId: key.team_id, keyId: key.id })
    }

    // ── Usage: get team stats ────────────────────────────────────────────────
    if (action === "get_usage" || action === "getUsage") {
      const auth = verify(req.headers.authorization)
      if (!auth?.email) return res.status(401).json({ error: "Unauthorized" })
      const db = getDb()
      const userResult = await db.execute({ sql: "SELECT team_id FROM users WHERE email = ?", args: [auth.email] })
      const teamId = userResult.rows?.[0]?.team_id
      if (!teamId) return res.status(200).json({ totalUsage: 0, keys: [] })
      const keysResult = await db.execute({
        sql: "SELECT id, name, daily_usage_count, last_reset_date, revoked_at FROM api_keys WHERE team_id = ?",
        args: [teamId],
      })
      const today = new Date().toISOString().split("T")[0]
      let totalUsage = 0
      ...
      for (const k of keysResult.rows || []) {
        const usage = k.last_reset_date !== today ? 0 : k.daily_usage_count || 0
        totalUsage += usage
        keys.push({ id: k.id, name: k.name, usage, limit: 200, remaining: k.revoked_at ? 0 : Math.max(0, 200 - usage) })
      }
      return res.status(200).json({ totalUsage, keys, dailyLimit: 200 })
    }
  } catch (err) {
